...
public class ExportExcelUtil<T>{
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void exportExcel2003ForTotal(String title, String[] headers, Collection<T> dataset, OutputStream out, String pattern) {
        // 声明一个工作薄
        HSSFWorkbook workbook = new HSSFWorkbook();
        // 生成一个表格
        HSSFSheet sheet = workbook.createSheet(title);
                ...
                try {
                    ...
                    if (value instanceof Integer) {
                        ...
                        if (map.containsKey(fieldName)) {
                            map.put(fieldName, (Float)map.get(fieldName)+(Float)value);
                        }else {
                        ...
                        cell.setCellValue(textValue);
                        if (map.containsKey(fieldName)) {
                            map.put(fieldName, (Double)map.get(fieldName)+(Double)value);
                        }else {
                            map.put(fieldName, value);
                        }
                    } else if (value instanceof Long) {
                        cell.setCellValue((Long) value);
                        if (map.containsKey(fieldName)) {
                            map.put(fieldName, (Long)map.get(fieldName)+(Long)value);
                        }else {
                            map.put(fieldName, value);
                        }
                    }
                    ...
                    if (textValue != null) {
                        ...
                        if (matcher.matches()) {
                            ...
                            cell.setCellValue(Double.parseDouble(textValue));
                            if (map.containsKey(fieldName)) {
                                map.put(fieldName, (Double)map.get(fieldName)+Double.parseDouble(textValue));
                            }else {
                                map.put(fieldName, Double.parseDouble(textValue));
                            }
                        } else {
                            richString = new HSSFRichTextString(textValue);
                            cell.setCellValue(richString);
                            if (headers[i].contains("金额")|| headers[i].contains("费") ){
                                if (map.containsKey(fieldName)) {
                                    map.put(fieldName, (Double)map.get(fieldName)+Double.parseDouble(textValue));
                                }else {
                                    map.put(fieldName, Double.parseDouble(textValue));
                            ...
                            if ( headers[i].contains("笔数")) {
                                if (map.containsKey(fieldName)) {
                                    map.put(fieldName, (Integer)map.get(fieldName)+Integer.parseInt(textValue));
                                }else {
                                    map.put(fieldName, Integer.parseInt(textValue));
                                }
                            }
                ...
                try {
                    ...
                    if (headers[i].contains("金额")|| headers[i].contains("费") ) {
                        if (map.get(fieldName) != null) {
                            cell.setCellValue(String.format("%.2f", map.get(fieldName)));
                        }
                    }else {
                        if (map.get(fieldName) != null) {
                            cell.setCellValue(String.valueOf(map.get(fieldName)));
                        }
                    }
